...
namespace MideFrameWork_AppDataInterface
{
    ...
    /// GetMyTeamList 的摘要说明
    /// </summary>
    public class GeVenderList : BaseForm, IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                IList<WG_VenderEntity> VenderList = DataProvider.GetInstance().GetWG_VenderList(20, int.Parse(PageIndex), " WHERE " + whereStr, " CreateDate Desc ", out recordCount, out pageCount);

                //如果所有数据分页后的总页数比请求的页数小，则返回空。
                if (int.Parse(PageIndex) > pageCount)
                    VenderList.Clear();

                if (VenderList != null && VenderList.Count > 0)
                {
                    jbo.code = 0;
                    jbo.data = VenderList;
                    jbo.success = true;
                    jbo.message = "成功获取商家列表列表。";
                }
                else
                {
                    //不存在
