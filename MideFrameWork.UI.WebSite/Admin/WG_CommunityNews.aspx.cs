...
{
    public partial class WG_CommunityNewsList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_CommunityNewsEntity> WG_CommunityNewsList = DataProvider.GetInstance().GetWG_CommunityNewsList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_CommunityNewsRepeat.DataSource = WG_CommunityNewsList;
            WG_CommunityNewsRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_CommunityNewsRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_CommunityNewsList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_CommunityNewslist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
