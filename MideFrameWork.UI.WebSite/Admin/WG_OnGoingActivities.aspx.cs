...
{
    public partial class WG_OnGoingActivitiesList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_OnGoingActivitiesEntity> WG_OnGoingActivitiesList = DataProvider.GetInstance().GetWG_OnGoingActivitiesList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_OnGoingActivitiesRepeat.DataSource = WG_OnGoingActivitiesList;
            WG_OnGoingActivitiesRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_OnGoingActivitiesRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_OnGoingActivitiesList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_OnGoingActivitieslist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
