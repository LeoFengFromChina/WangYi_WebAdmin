...
{
    public partial class LogList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<LogEntity> LogList = DataProvider.GetInstance().GetLogList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            LogRepeat.DataSource = LogList;
            LogRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(LogRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteLogList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("Loglist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
