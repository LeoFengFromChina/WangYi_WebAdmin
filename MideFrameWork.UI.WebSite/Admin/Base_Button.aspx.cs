...
{
    public partial class Base_ButtonList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<Base_ButtonEntity> Base_ButtonList = DataProvider.GetInstance().GetBase_ButtonList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            Base_ButtonRepeat.DataSource = Base_ButtonList;
            Base_ButtonRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(Base_ButtonRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteBase_ButtonList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("Base_Buttonlist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
