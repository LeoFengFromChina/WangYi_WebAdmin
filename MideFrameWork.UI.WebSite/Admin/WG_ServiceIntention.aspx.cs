...
{
    public partial class WG_ServiceIntentionList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_ServiceIntentionEntity> WG_ServiceIntentionList = DataProvider.GetInstance().GetWG_ServiceIntentionList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_ServiceIntentionRepeat.DataSource = WG_ServiceIntentionList;
            WG_ServiceIntentionRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_ServiceIntentionRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_ServiceIntentionList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_ServiceIntentionlist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
