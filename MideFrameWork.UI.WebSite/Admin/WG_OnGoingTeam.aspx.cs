...
{
    public partial class WG_OnGoingTeamList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_OnGoingTeamEntity> WG_OnGoingTeamList = DataProvider.GetInstance().GetWG_OnGoingTeamList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_OnGoingTeamRepeat.DataSource = WG_OnGoingTeamList;
            WG_OnGoingTeamRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_OnGoingTeamRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_OnGoingTeamList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_OnGoingTeamlist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
