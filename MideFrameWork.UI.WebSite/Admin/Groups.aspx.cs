...
{
    public partial class GroupsList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<GroupsEntity> GroupsList = DataProvider.GetInstance().GetGroupsList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            GroupsRepeat.DataSource = GroupsList;
            GroupsRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(GroupsRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteGroupsList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("Groupslist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
