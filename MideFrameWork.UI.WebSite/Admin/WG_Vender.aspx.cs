...
{
    public partial class WG_VenderList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_VenderEntity> WG_VenderList = DataProvider.GetInstance().GetWG_VenderList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_VenderRepeat.DataSource = WG_VenderList;
            WG_VenderRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_VenderRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_VenderList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_Venderlist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
