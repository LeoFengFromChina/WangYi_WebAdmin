...
{
    public partial class FriendLinksList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<FriendLinksEntity> FriendLinksList = DataProvider.GetInstance().GetFriendLinksList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            FriendLinksRepeat.DataSource = FriendLinksList;
            FriendLinksRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(FriendLinksRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteFriendLinksList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("FriendLinkslist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
