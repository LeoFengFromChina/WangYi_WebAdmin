...
{
    public partial class Base_PrivilegeList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<Base_PrivilegeEntity> Base_PrivilegeList = DataProvider.GetInstance().GetBase_PrivilegeList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            Base_PrivilegeRepeat.DataSource = Base_PrivilegeList;
            Base_PrivilegeRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(Base_PrivilegeRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteBase_PrivilegeList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("Base_Privilegelist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
