...
{
    public partial class WG_TeamList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_TeamEntity> WG_TeamList = DataProvider.GetInstance().GetWG_TeamList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_TeamRepeat.DataSource = WG_TeamList;
            WG_TeamRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_TeamRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_TeamList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_Teamlist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
