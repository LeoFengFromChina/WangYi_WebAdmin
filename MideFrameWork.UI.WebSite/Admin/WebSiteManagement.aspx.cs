...
{
    public partial class WebSiteManagementList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WebSiteManagementEntity> WebSiteManagementList = DataProvider.GetInstance().GetWebSiteManagementList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WebSiteManagementRepeat.DataSource = WebSiteManagementList;
            WebSiteManagementRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WebSiteManagementRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWebSiteManagementList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WebSiteManagementlist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
