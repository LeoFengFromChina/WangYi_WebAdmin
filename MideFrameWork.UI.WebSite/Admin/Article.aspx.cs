...
{
    public partial class ArticleList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<ArticleEntity> ArticleList = DataProvider.GetInstance().GetArticleList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            ArticleRepeat.DataSource = ArticleList;
            ArticleRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(ArticleRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteArticleList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("Articlelist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
