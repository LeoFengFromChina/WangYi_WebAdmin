...
{
    public partial class WG_GiftsList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_GiftsEntity> WG_GiftsList = DataProvider.GetInstance().GetWG_GiftsList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_GiftsRepeat.DataSource = WG_GiftsList;
            WG_GiftsRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_GiftsRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_GiftsList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_Giftslist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
