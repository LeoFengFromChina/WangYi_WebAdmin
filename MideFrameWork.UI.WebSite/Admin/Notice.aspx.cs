...
{
    public partial class NoticeList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<NoticeEntity> NoticeList = DataProvider.GetInstance().GetNoticeList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            NoticeRepeat.DataSource = NoticeList;
            NoticeRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(NoticeRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteNoticeList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("Noticelist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
