...
{
    public partial class WG_RegionList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_RegionEntity> WG_RegionList = DataProvider.GetInstance().GetWG_RegionList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_RegionRepeat.DataSource = WG_RegionList;
            WG_RegionRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_RegionRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_RegionList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_Regionlist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
