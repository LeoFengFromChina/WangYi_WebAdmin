...
{
    public partial class GlobalMenuList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<GlobalMenuEntity> GlobalMenuList = DataProvider.GetInstance().GetGlobalMenuList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            GlobalMenuRepeat.DataSource = GlobalMenuList;
            GlobalMenuRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(GlobalMenuRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteGlobalMenuList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("GlobalMenulist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
