...
{
    public partial class UsersList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<UsersEntity> UsersList = DataProvider.GetInstance().GetUsersList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            UsersRepeat.DataSource = UsersList;
            UsersRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(UsersRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteUsersList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("Userslist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
