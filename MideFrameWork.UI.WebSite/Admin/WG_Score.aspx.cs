...
{
    public partial class WG_ScoreList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_ScoreEntity> WG_ScoreList = DataProvider.GetInstance().GetWG_ScoreList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_ScoreRepeat.DataSource = WG_ScoreList;
            WG_ScoreRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_ScoreRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_ScoreList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_Scorelist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
