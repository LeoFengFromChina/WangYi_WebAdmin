...
{
    public partial class WG_HelpRequestList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_HelpRequestEntity> WG_HelpRequestList = DataProvider.GetInstance().GetWG_HelpRequestList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_HelpRequestRepeat.DataSource = WG_HelpRequestList;
            WG_HelpRequestRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_HelpRequestRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_HelpRequestList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_HelpRequestlist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
