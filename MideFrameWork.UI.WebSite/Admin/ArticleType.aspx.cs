...
{
    public partial class ArticleTypeList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<ArticleTypeEntity> ArticleTypeList = DataProvider.GetInstance().GetArticleTypeList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            ArticleTypeRepeat.DataSource = ArticleTypeList;
            ArticleTypeRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(ArticleTypeRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteArticleTypeList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("ArticleTypelist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
