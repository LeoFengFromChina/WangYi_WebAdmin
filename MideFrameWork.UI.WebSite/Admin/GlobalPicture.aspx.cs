...
{
    public partial class GlobalPictureList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<GlobalPictureEntity> GlobalPictureList = DataProvider.GetInstance().GetGlobalPictureList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            GlobalPictureRepeat.DataSource = GlobalPictureList;
            GlobalPictureRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(GlobalPictureRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteGlobalPictureList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("GlobalPicturelist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
