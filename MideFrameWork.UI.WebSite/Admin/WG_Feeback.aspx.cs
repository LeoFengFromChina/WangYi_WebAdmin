...
{
    public partial class WG_FeebackList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_FeebackEntity> WG_FeebackList = DataProvider.GetInstance().GetWG_FeebackList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_FeebackRepeat.DataSource = WG_FeebackList;
            WG_FeebackRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_FeebackRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_FeebackList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_Feebacklist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
