...
{
    public partial class WG_OnGoingGiftsList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_OnGoingGiftsEntity> WG_OnGoingGiftsList = DataProvider.GetInstance().GetWG_OnGoingGiftsList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_OnGoingGiftsRepeat.DataSource = WG_OnGoingGiftsList;
            WG_OnGoingGiftsRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_OnGoingGiftsRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_OnGoingGiftsList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_OnGoingGiftslist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
