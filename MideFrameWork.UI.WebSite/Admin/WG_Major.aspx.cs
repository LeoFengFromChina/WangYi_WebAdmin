...
{
    public partial class WG_MajorList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_MajorEntity> WG_MajorList = DataProvider.GetInstance().GetWG_MajorList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_MajorRepeat.DataSource = WG_MajorList;
            WG_MajorRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_MajorRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_MajorList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_Majorlist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
