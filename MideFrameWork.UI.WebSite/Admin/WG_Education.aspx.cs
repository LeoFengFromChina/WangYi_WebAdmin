...
{
    public partial class WG_EducationList : BaseForm
    {
        ...
        void myPagination_Bind(object sender, EventArgs e)
        {
            ...
            int totalPages = 0;
            myPagination.Query = myQueryHelper;
            IList<WG_EducationEntity> WG_EducationList = DataProvider.GetInstance().GetWG_EducationList(myPagination.PageSize,
                myPagination.CurrentPageIndex,
                myPagination.Query.WhereStr,
                myPagination.Query.OrderBy,
                out totalRecords,
                out totalPages);
            WG_EducationRepeat.DataSource = WG_EducationList;
            WG_EducationRepeat.DataBind();
        ...
        protected void lbt_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                string ids = GetSelectedAsString(WG_EducationRepeat);
                if (ids.Length > 0)
                {
                    DataProvider.GetInstance().DeleteWG_EducationList(ids);
                    Alert("操作成功!");
            ...
            {
                WriteLog("WG_Educationlist", "lbt_Delete_Click", ex.ToString(), Common.DailyUtility.MideSmsType.LogType.SystemLog);
